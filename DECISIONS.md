...
### Latency round 2 — user asked to push further toward sub-10s

Live testing after round 1 showed `resources_ms` was still 5-15s in the worst cases, from three additional sources not yet addressed:

- **No ceiling on the web-fallback stage as a whole.** The per-call/per-batch timeouts only bounded one DDG call; a roadmap with many short nodes could still stack up several waves' worth of waiting with no aggregate cap. Added `WEB_FALLBACK_STAGE_DEADLINE_SECONDS = 4.0` wrapping the entire fallback stage (`_resolve_web_fallback`, extracted so it could be `asyncio.wait_for`'d as one unit) — whatever hasn't resolved by the deadline gets the honest fallback link instead of continuing to wait. Confirmed live (`web_fallback_stage_deadline_exceeded` firing and correctly capping the stage).
- **`DDG_CALL_TIMEOUT_SECONDS=2.0` (set in round 1) was too aggressive** — a healthy isolated `ddgs` call measured ~2.4s, so 2.0s was timing out even healthy calls, never giving DDG a real chance to succeed. Raised to 2.5s; the circuit breaker (not the per-call timeout) is what now bounds the cost of DDG being unhealthy, so this could be relaxed without losing the round-1 fix.
- **Reranking cost itself, independent of any web fallback.** `CANDIDATE_POOL_SIZE=50` meant every node's cross-encoder pass scored 50 fused candidates even when corpus-only, measurably adding to `resources_ms` before web fallback was ever considered. Lowered to 20 — RRF's top ~20 fused candidates are already a 278-item corpus's best guesses; scoring 30 more past that for a 3-slot result was past diminishing returns.
- Also loosened `WEAK_SCORE_THRESHOLD` from `0.0` to `-0.5`: the corpus is course/doc-level (broad resources), so a genuinely relevant broad resource for a narrow node sometimes scored just under the cross-encoder's exact zero boundary — pushing to web fallback (the dominant cost) even when a decent corpus match already existed. Still a real relevance judgment, not "return whatever's least bad."
- `WEB_SEARCH_MAX_CONCURRENCY` raised 4→5 and `WEB_SEARCH_TIMEOUT_SECONDS` lowered 6.0→4.0 to match the new stage deadline (redundant with it in the aggregate case, still a sane per-node ceiling for the single-short-node case).

**Measured live, before → after this round**: `resources_ms` dropped from a 10-31s range down to consistently 3-8s (many runs now 3-6s, worst observed 7.8s across several different goals) — a real, repeated improvement, not a one-off. Full totals: "Learn UX Design" completed in **9.71s end to end** (under the sub-10s target); "Learn Cybersecurity" resolved resources in 3.4s with excellent quality (23/24 resource slots were real Course/Interactive Course results, not fallback links) but the *LLM call alone* took 12.4s that run, pushing the total to 15.8s.

**Where the remaining variance actually lives now**: `llm_ms` (the `gpt-4o-mini` structured-output call) ranged **5.6s to 12.4s** across this session's live runs — that's now the dominant and least controllable factor, an external API call's own latency variance, not something the retrieval architecture can bound. Sub-10s is reliably achievable when the LLM call lands in its typical range (~5-7s observed most often) plus the now-tightened resource stage; it isn't guaranteed on an LLM-call outlier, and no change made here claims otherwise. Not pursued further in this pass: capping/retrying the LLM call itself, or switching `ROADMAP_MODEL` to something faster - flagged as the next lever if the user wants to push further, since it's more of a product/cost decision (which model, what quality tradeoff) than a pure engineering one.
